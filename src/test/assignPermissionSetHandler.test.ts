...
vi.mock("@aws-sdk/client-sso-admin", () => ({
  ...
  CreateAccountAssignmentCommand: class {
    constructor(public input: unknown) {}
  },
...
const { handler } = await import(
  "../../amplify/functions/accessRequests/assignPermissionSetHandler"
);
...
describe("assignPermissionSetHandler", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("calls CreateAccountAssignment with correct parameters", async () => {
  ...
  it("updates DynamoDB status to ACTIVE on success", async () => {
    mockSsoSend.mockResolvedValue({
      AccountAssignmentCreationStatus: { Status: "SUCCEEDED" },
    });
    ...
    expect(cmd.input.ExpressionAttributeValues[":s"]).toBe("ACTIVE");
  });

  it("returns the input so subsequent Step Function states have all fields", async () => {
    mockSsoSend.mockResolvedValue({
      AccountAssignmentCreationStatus: { Status: "SUCCEEDED" },
    });
  ...
  it("throws when CreateAccountAssignment returns FAILED status", async () => {
    mockSsoSend.mockResolvedValue({
    ...
    await expect(handler(BASE_INPUT)).rejects.toThrow(
      "CreateAccountAssignment failed for request req-1: Permission denied"
    );
    // DynamoDB should not be updated when assignment fails
